...
def train(train_loader, val_loader, config):
    ...
    with open(log_path, 'w') as log_writer:
        for epoch in range(epochs):  # loop over the dataset multiple times
            ...
            current_score, current_far_frr10 = val_step(net, val_loader, config)
...
if __name__ == '__main__':
    ...
    best_far_frr10, best_checkpoint_name = (100.0, 10.0), None
    best_mean_far_frr10 = None

    for (score, checkpoint_name) in score_heap:
        mean_far_frr10 = (-score[0], -score[1])
        message = f'check checkpoint name: {checkpoint_name}'
        print(message)
        ...
        val_score, val_far_frr10 = val_step(net, val_loader, config, iters=1000, m=90, q=0.1, p=0.8)
        
        message = (
            f'mean val far@frr: {mean_far_frr10[0] * 100:.4f}%@{mean_far_frr10[1] * 100:.2f}% '
            f'p08 val far@frr: {val_far_frr10[0] * 100:.4f}%@{val_far_frr10[1] * 100:.2f}% '
        )
        print(message)
        if val_far_frr10 < best_far_frr10:
            best_far_frr10 = val_far_frr10
            best_checkpoint_name = checkpoint_name
            best_mean_far_frr10 = mean_far_frr10

    message = (
        f'best checkpoint name: {best_checkpoint_name}\n'
        f'best mean val far@frr: {best_mean_far_frr10[0] * 100:.4f}%@{best_mean_far_frr10[1] * 100:.2f}% '
        f'best p08 val far@frr: {best_far_frr10[0] * 100:.4f}%@{best_far_frr10[1] * 100:.2f}%'
    )
    print(message)

    test_log_path = os.path.join(experiment_path, 'reval_log.txt')
